...
modelo_lineal <- lm(Precio ~ Categoria + Marca + Supermercado, data = productos)
...
# Agregamos una nueva columna para identificar si el nombre del producto contiene 'aceite'
productos_actualizados <- productos_actualizados %>%
  mutate(contiene_aceite = ifelse(grepl("aceite", Nombre, ignore.case = TRUE), "Con Aceite", "Sin Aceite"))

# Calculamos el precio medio general excluyendo NA
precio_medio_general <- mean(productos_actualizados$Precio, na.rm = TRUE)

# Subconjunto de productos que contienen 'aceite' en el nombre
productos_con_aceite <- productos_actualizados %>%
  filter(contiene_aceite == "Con Aceite") %>%
  pull(Precio)

# Prueba de normalidad
shapiro_test_aceite <- shapiro.test(productos_con_aceite)
print(shapiro_test_aceite)

# Dependiendo de la normalidad, realizar t-test o Wilcoxon test
if (shapiro_test_aceite$p.value > 0.05) {
  t_test_result <- t.test(productos_con_aceite, mu = precio_medio_general)
  print(t_test_result)
} else {
  print("Distribución no normal, aplicando prueba no paramétrica.")
  wilcox_test_result <- wilcox.test(productos_con_aceite, mu = precio_medio_general, alternative = "greater")
